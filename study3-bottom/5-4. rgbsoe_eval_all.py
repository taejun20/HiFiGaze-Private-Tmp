...
def find_rgbsoe_checkpoints():
    """Find all RGBSOE checkpoints in the best directory"""
    pattern = os.path.join(BEST_CHECKPOINTS_DIR, "RGBSOE_*.pt")
...
def main():
    ...
    # Find all RGBSOE checkpoints
    rgbsoe_checkpoints = find_rgbsoe_checkpoints()
    print(f"Found {len(rgbsoe_checkpoints)} RGBSOE checkpoints:")
    for cp in rgbsoe_checkpoints:
        print(f"  - {os.path.basename(cp)}")

    if not rgbsoe_checkpoints:
        print("No RGBSOE checkpoints found!")
        return

    # Evaluate each checkpoint
    all_aggregated_results = []
    
    for checkpoint_path in rgbsoe_checkpoints:
        try:
    ...
    # Save to CSV
    csv_path = f"{BEST_CHECKPOINTS_DIR}/RGBSOE_eval_pall.csv"
