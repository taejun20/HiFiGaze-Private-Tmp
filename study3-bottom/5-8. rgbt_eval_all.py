...
def find_rgbt_checkpoints():
    """Find all RGBT checkpoints in the best directory"""
    pattern = os.path.join(BEST_CHECKPOINTS_DIR, "RGBT_*.pt")
...
def main():
    ...
    # Find all RGBT checkpoints
    rgbt_checkpoints = find_rgbt_checkpoints()
    print(f"Found {len(rgbt_checkpoints)} RGBT checkpoints:")
    for cp in rgbt_checkpoints:
        print(f"  - {os.path.basename(cp)}")

    if not rgbt_checkpoints:
        print("No RGBT checkpoints found!")
        return

    # Evaluate each checkpoint
    all_aggregated_results = []
    
    for checkpoint_path in rgbt_checkpoints:
        try:
    ...
    # Save to CSV
    csv_path = f"{BEST_CHECKPOINTS_DIR}/RGBT_eval_pall.csv"
