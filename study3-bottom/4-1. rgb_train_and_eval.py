...
class GazePTDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):        
        ...
        return l_eye, r_eye, eyecorner_lmk, gt
...
def evaluate_model(model, loader, device):
    ...
    with torch.no_grad():
        for l_eye, r_eye, eyecorner_lmk, gaze in tqdm(loader, desc="Evaluating", position=1):
            l_eye, r_eye = l_eye.to(device), r_eye.to(device)
            eyecorner_lmk, gaze = eyecorner_lmk.to(device), gaze.to(device)
...
def train_and_evaluate(train_subjects, val_subjects, eval_subjects, device):
    ...
    model = GazeModelRGB(generator=model_init_generator).to(device)
    ...
    for epoch_idx, epoch in enumerate(range(NUM_EPOCHS)):
        ...
        for batch_idx, (l_eye, r_eye, eyecorner_lmk, gaze) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{NUM_EPOCHS}", position=0)):
            l_eye, r_eye = l_eye.to(device), r_eye.to(device)
            eyecorner_lmk, gaze = eyecorner_lmk.to(device), gaze.to(device)
...
def main():
    ...
    for iteration in [1,2,3,4,5,6,7,8,9]:
