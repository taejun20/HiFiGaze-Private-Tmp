...
# Initialize results dictionary for aggregating by trial
trial_aggregated = defaultdict(lambda: {'soe_counts': [], 'non_soe_counts': [], 'total_sessions': 0})
...
for subject_num in tqdm(SUBJECTS, desc="Processing subjects"):
    ...
    for session_num in SESSIONS:
        ...
        for trial in sorted(trials):
            ...
            # Aggregate results by trial number
            trial_aggregated[trial]['soe_counts'].append(soe_count)
            trial_aggregated[trial]['non_soe_counts'].append(non_soe_count)
            trial_aggregated[trial]['total_sessions'] += 1
            
            print(f"      Trial {trial}: SoE={soe_count}, non-SoE={non_soe_count}")

# Calculate aggregated statistics by trial
aggregated_results = []
for trial in sorted(trial_aggregated.keys()):
    data = trial_aggregated[trial]
...
csv_filename = 'soe_count_by_trial_aggregated.csv'
with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['trial', 'total_sessions', 'total_soe_count', 'total_non_soe_count', 'avg_soe_count', 'avg_non_soe_count']
...
print(f"Total trials analyzed: {len(aggregated_results)}")
...
for result in aggregated_results:
    ...
    print(f"Trial {trial}: {sessions} sessions, {total_frames} total frames")
