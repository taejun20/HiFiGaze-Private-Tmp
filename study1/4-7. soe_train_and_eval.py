...
import random
import wandb
from GazeModelSOE import GazeModelSOE
import warnings
            ...
            for session in SESSIONS:
                ...
                for l_eye_path in l_eye_paths:
                    ...
                    if r_eye_path.exists() and json_file.exists():
                        # Read JSON to check SOE values
                        with open(json_file, 'r') as f:
                            json_data = json.load(f)
                        
                        # Only append if at least one eye's SOE is not masked
                        if not (json_data["l_iris_to_soe_center_x_norm"] == -999 and json_data["r_iris_to_soe_center_x_norm"] == -999):
...
def evaluate_model(model, loader, device, train_val_eval_str, epoch, dataset_type, val_error_4f_str):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(loader, desc="Evaluating", position=1)):
            ...
            pred = model(soe_cues, eyecorner_lmk)
