...
# preventing thread deadlock - freezing the server
# cv2.setNumThreads(0)
...
def train_and_evaluate(train_subjects, val_subjects, eval_subjects, device):
    ...
    model = GazeModelRGBSOE().to(device)   
...
def main():
    parser = argparse.ArgumentParser(description=f'Train and evaluate gaze prediction model')
