...
import random
import wandb
from GazeModelRGBHSOE import GazeModelRGBHSOE
import warnings
...
class GazePTDataset(torch.utils.data.Dataset):
    def __init__(self, subjects):
        ...
        for subject in subjects:
            for session in SESSIONS:
                ...
                heatmap_dir = Path(f"preprocessed/input_heatmap/p{subject}/s{session}")
                ...
                for l_eye_path in l_eye_paths:
                    ...
                    l_heatmap_path = heatmap_dir / f"{frame_num}_left.png"
                    r_heatmap_path = heatmap_dir / f"{frame_num}_right.png"
                    ...
                    if r_eye_path.exists() and json_file.exists():
                        self.data_samples.append({
                            ...
                            'l_heatmap_path': l_heatmap_path,
                            'r_heatmap_path': r_heatmap_path
                        })
                    else:
        ...
        self.heatmap_transform = transforms.Compose([
            transforms.ToPILImage(),
            transforms.Resize((55, 40)),  # (height=55, width=40)
            transforms.ToTensor()         # -> [1, 55, 40], values in [0,1]
        ])
    ...
    def __getitem__(self, idx):        
        ...
        l_heatmap_img = cv2.imread(str(sample['l_heatmap_path']), cv2.IMREAD_GRAYSCALE)
        r_heatmap_img = cv2.imread(str(sample['r_heatmap_path']), cv2.IMREAD_GRAYSCALE)

        l_heatmap = self.heatmap_transform(l_heatmap_img)   # [1, 55, 40]
        r_heatmap = self.heatmap_transform(r_heatmap_img)   # [1, 55, 40]
...
def evaluate_model(model, loader, device, train_val_eval_str, epoch, dataset_type, val_error_4f_str):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(loader, desc="Evaluating", position=1)):
            ...
            l_heatmap = batch['l_heatmap'].to(device)
            r_heatmap = batch['r_heatmap'].to(device)
            soe_cues = batch['soe_cues'].to(device)
            eyecorner_lmk = batch['eyecorner_lmk'].to(device)
            gt = batch['gt'].to(device)
            
            pred = model(l_eye, r_eye, l_heatmap, r_heatmap, soe_cues, eyecorner_lmk)
...
def train_and_evaluate(train_subjects, val_subjects, eval_subjects, device):
    ...
    model = GazeModelRGBHSOE().to(device)   
