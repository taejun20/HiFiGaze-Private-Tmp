...
def segment_iris(frame, landmarks, subject, session, frame_name, irisbox_center_frame, irisbox_width, irisbox_height, debug_iris_segment_dir, is_left):
    ...
    eyepatch = frame[eyepatch_top:eyepatch_bottom, eyepatch_left:eyepatch_right].copy()
    ...
    if DEBUG_SAVE_MODE:
        ...
        eyepatch_debug_width = round(irisbox_width * EYE_CROP_SCALE_FACTOR)
        eyepatch_debug_height = eyepatch_debug_width // 2  
        eyepatch_debug_left = irisbox_center_frame[0] - eyepatch_debug_width // 2
        eyepatch_debug_right = irisbox_center_frame[0] + eyepatch_debug_width // 2
        eyepatch_debug_top = irisbox_center_frame[1] - eyepatch_debug_height // 2
        eyepatch_debug_bottom = irisbox_center_frame[1] + eyepatch_debug_height // 2
        if eyepatch_debug_left < 0 or eyepatch_debug_right > frame_width or eyepatch_debug_top < 0 or eyepatch_debug_bottom > frame_height:
            print(f" Warning: Eye patch for iris segmentation gets out of the frame, skipping: subject: {subject}, session: {session}, frame: {frame_name}, is_left: {is_left}")
            return None, None, None
        eyepatch_debug = frame[eyepatch_debug_top:eyepatch_debug_bottom, eyepatch_debug_left:eyepatch_debug_right].copy()
        ...
        # Draw MediaPipe iris center and radius in cyan
        mp_iris_center = (round(landmarks[LEFT_IRIS_CENTER].x * frame_width) - eyepatch_debug_left if is_left else round(landmarks[RIGHT_IRIS_CENTER].x * frame_width) - eyepatch_debug_left,
                         round(landmarks[LEFT_IRIS_CENTER].y * frame_height) - eyepatch_debug_top if is_left else round(landmarks[RIGHT_IRIS_CENTER].y * frame_height) - eyepatch_debug_top)
        
        # Calculate MediaPipe iris radius using inner and outer iris points
        if is_left:
            mp_iris_radius = round(math.sqrt(((landmarks[LEFT_IRIS_OUTER].x - landmarks[LEFT_IRIS_INNER].x) * frame_width)**2 + 
                                           ((landmarks[LEFT_IRIS_OUTER].y - landmarks[LEFT_IRIS_INNER].y) * frame_height)**2)) // 2
        else:
            mp_iris_radius = round(math.sqrt(((landmarks[RIGHT_IRIS_OUTER].x - landmarks[RIGHT_IRIS_INNER].x) * frame_width)**2 + 
                                           ((landmarks[RIGHT_IRIS_OUTER].y - landmarks[RIGHT_IRIS_INNER].y) * frame_height)**2)) // 2
            
        # Draw MediaPipe iris circle and center in purple
        cv2.circle(eyepatch_debug, mp_iris_center, 3, (255, 0, 255), -1)  # Purple center point

        
        xxx = (initial_center_eyepatch[0] + 7 - eyepatch_debug_left + eyepatch_left, initial_center_eyepatch[1] + 2 - eyepatch_debug_top + eyepatch_top)
        # Draw r1, r2, r3 circles used for GrabCut initialization
        cv2.circle(eyepatch_debug, xxx, r3, (128, 128, 128), 2)  # Gray
        cv2.circle(eyepatch_debug, xxx, r2, (128, 128, 128), 2)  # Gray
        cv2.circle(eyepatch_debug, xxx, r1, (128, 128, 128), 2)  # Gray
        # Draw full iris contour in green
        # Adjust iris contour points coordinates
        adjusted_contour_points = iris_contour_points + np.array([-eyepatch_debug_left + eyepatch_left, -eyepatch_debug_top + eyepatch_top])
        cv2.drawContours(eyepatch_debug, [adjusted_contour_points], -1, (0, 255, 0), 2)  # Green

        # Draw fitted circle in red (using the results from earlier circle fitting)
        cv2.circle(eyepatch_debug, (grabcut_iris_center_eyepatch[0] - eyepatch_debug_left + eyepatch_left, grabcut_iris_center_eyepatch[1] + 2 - eyepatch_debug_top + eyepatch_top), grabcut_iris_radius, (0, 255, 255), 2)  # Red circle
        ...
        if ransac_inliers is not None:  # Only draw points if RANSAC succeeded
            for point in ransac_inliers:
                cv2.circle(eyepatch_debug, (round(point[0] - eyepatch_debug_left + eyepatch_left), round(point[1] - eyepatch_debug_top + eyepatch_top)), 1, (255, 0, 0), 2)  # Blue
        
        # Draw final iris center in red
        cv2.circle(eyepatch_debug, (grabcut_iris_center_eyepatch[0] - eyepatch_debug_left + eyepatch_left, grabcut_iris_center_eyepatch[1] + 2 - eyepatch_debug_top + eyepatch_top), 3, (0, 255, 255), -1)  # Red
...
def fit_circle_ransac(points, max_iterations=300, threshold=2.0, min_inliers=0.6):
    """
...
def find_soe(frame, landmarks,
    ...
    if iriscrop_left < 0 or iriscrop_right > frame_width or iriscrop_top < 0 or iriscrop_bottom > frame_height:
        ...
        return -999, -999
    iriscrop = frame[iriscrop_top:iriscrop_bottom, iriscrop_left:iriscrop_right].copy()
