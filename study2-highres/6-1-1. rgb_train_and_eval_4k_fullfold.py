...
def build_lopo_folds(subjects: list[int]) -> list[dict]:
    """
    22 leave-one-participant-out folds.
    eval: one held-out subject; val: previous 3 in circular subject order; train: the rest (18).
    """
    n = len(subjects)
    folds = []
    for i in range(n):
        eval_subjects = [subjects[i]]
        val_subjects = [subjects[(i - 1) % n], subjects[(i - 2) % n], subjects[(i - 3) % n]]
        hold_out = set(eval_subjects) | set(val_subjects)
        train_subjects = [s for s in subjects if s not in hold_out]
        folds.append(
            {
...
FOLDS = build_lopo_folds(SUBJECTS)

WandB_PROJECT = "260516_4K_fullfold"
TYPE = "RGB4K"
