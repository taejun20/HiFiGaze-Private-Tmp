"""
Load RGB_1400 pretrained checkpoint (1400×700 eye crops) and evaluate each training subject.

Writes per subject under the checkpoint directory (or --out-dir):
  - RGB_1400_p{X}_eval.csv
  - RGB_1400_p{X}_pretrain_scatter.png
"""
...
DEFAULT_CKPT = CHECKPOINT_DIR / (
    "RGB_4k_train_p1,2,3,4,5,6,7_val_p8,9_eval_p10_Epoch12_ValError1.290_EvalError1.152.pt"
)
...
def evaluate_all_subjects(
    ...
    *,
    ...
    for sid in subjects:
        ...
        csv_path = out_dir / f"RGB_1400_p{sid}_eval.csv"
        ...
        plot_path = out_dir / f"RGB_1400_p{sid}_pretrain_scatter.png"
        _save_pred_scatter(
            pred_xs,
            pred_ys,
            plot_path,
            title=f"RGB_1400 p{sid} pretrained preds (n={len(rows)})",
        )
...
def main():
    parser = argparse.ArgumentParser(description="Eval RGB_1400 pretrained model per subject")
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=DEFAULT_CKPT,
        help="Path to RGB_1400_train_...pt",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
