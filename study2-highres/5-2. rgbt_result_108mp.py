...
OUT_CSV = Path("rgbt_study2_result.csv")

# Column order and allowed resolution tags in checkpoint filenames
...
_CKPT_RE = re.compile(
    r"^RGBT_(?P<res>[^_]+)_(?P<run_tag>train_p[^_]+_val_p[^_]+_eval_p[^_]+)_"
...
def main() -> None:
    ...
    # Output grid: rows p1..p10, cols RESOLUTIONS
    results: dict[tuple[int, str], float] = {}

    for (res, eval_p), run in best.items():
        eval_csv_name = f"Eval_RGBT_{res}_{run.run_tag}_Epoch{run.epoch}.csv"
