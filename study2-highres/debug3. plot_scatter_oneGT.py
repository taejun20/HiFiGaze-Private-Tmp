...
def _save_heatmap(
    ...
    *,
    ...
    for (ix, iy), pts in points_by_gtbin.items():
        if only_gtbin is not None and (ix, iy) != only_gtbin:
            continue
        if not pts:
            continue
        xs = [p[0] for p in pts]
...
def main() -> None:
    ...
    for res in RESOLUTIONS:
        ...
        # Choose the bottom-left anchor (largest gt_y, then smallest gt_x).
        bottom_left_key: tuple[int, int] | None = None
        if gt_anchor_by_gtbin:
            bottom_left_key = min(gt_anchor_by_gtbin.items(), key=lambda kv: (-kv[1][1], kv[1][0]))[0]

        # Save filtered rows (only this GT bin) into a new CSV.
        if bottom_left_key is not None:
            ...
            for pid in PARTICIPANTS:
                ...
                with open(ef.path, newline="", encoding="utf-8") as f:
                    ...
                    for r in reader:
                        ...
                        if k == bottom_left_key:
                            filtered_rows.append(r)

            if filtered_rows and fieldnames:
                out_csv = args.out_dir / f"filtered_RGB_{res}_bottom_left.csv"
                out_csv.parent.mkdir(parents=True, exist_ok=True)
                with open(out_csv, "w", newline="", encoding="utf-8") as f:
                    writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
                    writer.writeheader()
                    writer.writerows(filtered_rows)
                print(f"[{res}] saved filtered rows: {out_csv} (rows={len(filtered_rows)})")
            else:
                print(f"[{res}] no rows matched bottom-left bin; no filtered CSV written")

        out_path = args.out_dir / f"scatter_RGB_{res}.png"
        _save_heatmap(
            points_by_gtbin,
            gt_anchor_by_gtbin,
            bottom_left_key,
            ...
            width_cm=FRAME_W_CM,
            height_cm=FRAME_H_CM,
            title=f"RGB {res}: pred scatter (bottom-left GT anchor) (participants {len(used_p)})",
            vmin=None,
            vmax=None,
        )
        print(
            f"[{res}] saved {out_path} (participants: {used_p}; gt_bins: {len(points_by_gtbin)}; only={bottom_left_key})"
        )
