...
# Resume: skip all folds until eval p10,11,12,13,14; for that fold skip 4k..fhd and start at hd.
RESUME_EVAL_SET = frozenset({10, 11, 12, 13, 14})
RESUME_FIRST_RES_FOR_THAT_FOLD = "hd"
...
class GazePTDataset(torch.utils.data.Dataset):
    def __init__(self, subjects, res: str):
        ...
        for subject in subjects:
            ...
            template_dir = Path(f"study1_rawdata_processed/p{subject}/preprocessed/template")
            ...
            for metadata_file in metadata_files:
                ...
                # Check if corresponding right eye, landmark, and preprocessed template exist
                if (
                    l_eye_file.exists()
                    and r_eye_file.exists()
                    and landmark_file.exists()
    ...
    def __getitem__(self, idx):        
       ...
       # don't horizontally flip the left eye. The preprocessed left eye crop is already flipped.
        ...
        if template_img is None:
            raise FileNotFoundError(f"Could not load template: {sample['template_path']}")
        template_img = cv2.cvtColor(template_img, cv2.COLOR_BGR2RGB)
        template = torch.from_numpy(template_img).permute(2, 0, 1).contiguous().float().mul_(1.0 / 255.0)
...
def main():
    ...
    for fold_idx, fold in enumerate(FOLDS):
        ...
        if not resume_reached:
            if eval_frozen != RESUME_EVAL_SET:
                ev_sorted = ",".join(map(str, sorted(fold["eval"])))
                print(
                    f"Skipping fold {fold_idx + 1} (eval p{ev_sorted}) "
                    f"until eval p{','.join(map(str, sorted(RESUME_EVAL_SET)))}."
                )
                continue
            resume_reached = True
