...
class CameraController: NSObject, ObservableObject, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    private var uiStateLogsCalibration: [UIStateLogCalibration] = []
    private let uiStateLogsLockCalibration = NSLock()
    ...
    func saveUIStateLogs() {
        uiStateLogsLock.lock()
        let csvHeader = "subject,session,trial,px_x,px_y,px_norm_x,px_norm_y,backgroundA,backgroundB,dissolve,VSyncStartTimeOfPreviousUpdate,frameNum,frameEstimatedReadoutStartTime\n"
        let csvRows = uiStateLogs.map { log in
            "\(log.subject),\(log.session),\(log.trial),\(log.px_x),\(log.px_y),\(log.px_norm_x),\(log.px_norm_y),\(log.backgroundA),\(log.backgroundB),\(log.dissolve),\(log.VSyncStartTimeOfPreviousUpdate),\(log.frameNum),\(log.frameEstimatedReadoutStartTime)"
        }
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        else
        {
            ...
            if shouldCapture {
                ...
                uiStateLogsLockCalibration.lock()
                let startIndex = max(0, uiStateLogsCalibration.count - 5)
                let endIndex = uiStateLogsCalibration.count
                var closestIndex = -1
                var closestTimeDiff = Double.infinity
                for i in startIndex..<endIndex {
                    let uiStateLog = uiStateLogsCalibration[i]
                    let timeDiff = estimatedReadoutStartTime - uiStateLog.VSyncStartTimeOfPreviousUpdate
                ...
                if closestIndex > 0 {
                    if estimatedReadoutStartTime - uiStateLogsCalibration[closestIndex - 1].VSyncStartTimeOfPreviousUpdate > 0.1 {
                        uiStateLogsCalibration[closestIndex].frameNum = String(format: "%05d", frameCounter)
                        uiStateLogsCalibration[closestIndex].frameEstimatedReadoutStartTime = estimatedReadoutStartTime
                    }
                    else {
                        uiStateLogsCalibration[closestIndex - 1].frameNum = String(format: "%05d", frameCounter)
                        uiStateLogsCalibration[closestIndex - 1].frameEstimatedReadoutStartTime = estimatedReadoutStartTime
                    }
                }
                uiStateLogsLockCalibration.unlock()
    ...
    func recordUIState(subject: String, session: String, trial: Int, px_x: CGFloat, px_y: CGFloat, px_norm_x: CGFloat, px_norm_y: CGFloat, backgroundA: String, backgroundB: String, dissolve: CGFloat, VSyncStartTimeOfPreviousUpdate: CFTimeInterval) {
    ...
    func recordUIStateCalibration(subject: String, session: String, trial: Int, px_x: CGFloat, px_y: CGFloat, px_norm_x: CGFloat, px_norm_y: CGFloat, backgroundIndex: String, VSyncStartTimeOfPreviousUpdate: CFTimeInterval) {
        let uiStateLogCalibration = UIStateLogCalibration(
            subject: subject,
            session: session,
            trial: trial,
            px_x: px_x,
        ...
        uiStateLogsLockCalibration.lock()
        uiStateLogsCalibration.append(uiStateLogCalibration)
        uiStateLogsLockCalibration.unlock()
    }
    
    func saveUIStateLogsCalibration() {
        uiStateLogsLockCalibration.lock()
        let csvHeader = "subject,session,trial,px_x,px_y,px_norm_x,px_norm_y,backgroundIndex,VSyncStartTimeOfPreviousUpdate,frameNum,frameEstimatedReadoutStartTime\n"
        let csvRows = uiStateLogsCalibration.map { log in
            "\(log.subject),\(log.session),\(log.trial),\(log.px_x),\(log.px_y),\(log.px_norm_x),\(log.px_norm_y),\(log.backgroundIndex),\(log.VSyncStartTimeOfPreviousUpdate),\(log.frameNum),\(log.frameEstimatedReadoutStartTime)"
        }
        let csvString = csvHeader + csvRows.joined(separator: "\n")
        uiStateLogsLockCalibration.unlock()
        ...
        do {
            try csvString.write(to: csvURL, atomically: true, encoding: .utf8)
            print("✅ Calibration UI state logs saved to \(csvURL)")
        } catch {
            print("❌ Failed to write Calibration UI state logs: \(error)")
        }
    }
...
struct UIStateLog: Codable {
    ...
    let backgroundA: String
    let backgroundB: String
...
struct UIStateLogCalibration: Codable {
    let subject: String
    let session: String
    let trial: Int
    let px_x: CGFloat
    ...
    let px_norm_x: CGFloat
    let px_norm_y: CGFloat
    let backgroundIndex: String     // 0: black, 1: gray, 2: white
    let VSyncStartTimeOfPreviousUpdate: CFTimeInterval
    var frameNum: String  // Will be "-" initially, then updated with frame number
    var frameEstimatedReadoutStartTime: CFTimeInterval
}
