...
public class DataCollectionManager_MovingTarget : MonoBehaviour
{
    [SerializeField]
    ...
    float horizontalLocationCount = 8f;
    float verticalLocationCount = 6f;
    ...
    public IEnumerator MoveFromTo(GameObject obj, Vector3 start, Vector3 end)
    {
        ...
        obj.transform.position = end; // Snap to exact position at the end
        ...
        if (webcamTexture != null)
        {
           /* Texture2D snap = new Texture2D(webcamTexture.width, webcamTexture.height);
            for (int i=0; i< cameraFrameList.Count; i++)
            {
                ...
                string imgPath = imageSavePath + "frame_" + i.ToString("D4") + ".png";
                File.WriteAllBytes(imgPath, snap.EncodeToPNG());
            }*/
