...
if __name__ == "__main__":
    ...
    while True:
        ...
        #     # Known points: (20cm -> 558), (30cm -> 500), (40cm -> 458)
        #     if distance <= 30:
        #         val = 558 - (distance - 20) * (558 - 500) / (30 - 20)
        #     else:
        #         val = 500 - (distance - 30) * (500 - 458) / (40 - 30)
            
        #     val = int(val)  # Ensure integer value
            
        #     current_time = time.time()
        #     if not hasattr(cap, 'last_focus_time'):
        #         cap.last_focus_time = current_time
            
        #     time_interval = current_time - cap.last_focus_time
        #     cap.set_focus(val)
        #     cap.last_focus_time = current_time

        #     print(f"Eye-Cam Dist: {distance:.1f} cm, focus: {val}, interval: {time_interval:.3f}s")
        ...
        if key == ord("q"):
            ...
            for i in range(wait_frames):
                ...
                ret, frame = cap.read()
                end_time = time.time()
                print(f"Frame read took {end_time - start_time:.3f} seconds, cap.read result={ret}")
